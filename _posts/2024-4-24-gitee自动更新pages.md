...
&emsp;&emsp;**重要问题**：由于同步使用的是自带功能，与Page的同步方式不一致，如果先触发Page的同步，之后再触发仓库的同步会导致网页不会及时更新，一种方法是仓库同步也通过这种方法解决，这里我取了个巧，不知道能否解决这个问题，修改了更新的触发方式为Push，并且在gitee里面设置自动同步到Github，Github新建一个仓库，同时，我们去掉github主仓库的密钥，这样第一次更新不会成功，直接等到第二次再进行更新，就可以解决这个问题。
~~~
name: Sync
on:
  push:
